    ...
    public function validar()
    {
        ...
        if (!empty($data)) {
            if (password_verify($clave, $data['clave'])) {
                ...
                $rol = $data['rol'];
                $res = array('tipo' => 'success', 'mensaje' => 'BIENVENIDO AL SISTEMA', 'usuario' => $rol);
            } else {
    ...
    public function enviarCorreo($correo)
    {
        ...
        if (!empty($consulta)) {
            ...
            try {
                ...
                $mail->Username   = 'pagolabrian@gmail.com';                     //SMTP username
                $mail->Password   = 'rftcnhvgwshsjnnh';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('pagolabrian@gmail.com', 'INDEDA');
                $mail->addAddress($correo);

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'Restablecer clave';
                $mail->Body    = 'has pedido restablecer tu contraseña, si no has sido tu omite el mensaje <br><a href="'.BASE_URL.'principal/reset/'.$token.'">CLICK AQUI PARA CAMBIAR</a>';
            ...
            if ($nueva != $confirmar) {
                ...
                if ($token == $result['token']) {
                    ...
                    if ($data == 1) {
                        ...
                        $res = array('tipo' => 'error', 'mensaje' => 'ERROR AL RESTABLECER LA COMTRASEÑA');
                    }
